...
- (void)reloadData
{
    //NSLog(@"1- offsetx: %.f, fixedwidth: %.f, scrollwidth: %.f", self.offsetX, self.fixedWidth, self.scrollableWidth);
    ...
    //NSLog(@"2- offsetx: %.f, fixedwidth: %.f, scrollwidth: %.f", self.offsetX, self.fixedWidth, self.scrollableWidth);
...
//    NSInteger count = [self tableView:table numberOfRowsInSection:0];
//    
    [table beginUpdates];
    [table insertRowsAtIndexPaths:insertIndexPaths withRowAnimation:UITableViewRowAnimationNone];
    [table endUpdates];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGPoint point = _temporaryCell.baseView.scrollView.contentOffset;
    ...
    _temporaryCell.baseView.scrollView.contentOffset = CGPointMake(point.x, 0);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (cellView) {
        ...
        cellView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"cellGrey"]];
        [_temporaryCell updateConstraintsIfNeeded];
    }
