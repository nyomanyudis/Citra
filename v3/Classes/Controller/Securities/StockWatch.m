...
@property (weak, nonatomic) IBOutlet UIButton *buyNyoman;
...
- (void)viewWillDisappear:(BOOL)animated
{
    if(self.unsubscribe) {
        ...
        [[MarketFeed sharedInstance] unsubscribe:RecordTypeLevel3];
        [[MarketFeed sharedInstance] unsubscribe:RecordTypeStockHistory];
        [[MarketFeed sharedInstance] unsubscribe:RecordTypeKiLastTrade];
    }
}
...
//    self.table = [[SummaryTable alloc] initWithGridView:self.summaryTable];
...
- (void)showLevel2
{
    ...
    if(!self.level2)
        self.level2 = [[StockWatchLevel2V3 alloc] initWithFrame:CGRectMake(0, 0, self.container.frame.size.width, self.container.frame.size.height)];
    
    [self.container addSubview:self.level2.scrollView];
}
...
- (void)showLevel3
{
    ...
    if(!self.level3)
        self.level3 = [[StockWatchLevel3V3 alloc] initWithFrame:CGRectMake(0, 0, self.container.frame.size.width, self.container.frame.size.height)];
    
    [self.container addSubview:self.level3.scrollView];
}
...
- (void)showTrade
{
    ...
    if(!self.trade)
        self.trade = [[StockWatchTradeV3 alloc] initWithFrame:CGRectMake(0, 0, self.container.frame.size.width, self.container.frame.size.height)];
    
    [self.container addSubview:self.trade.scrollView];
}
...
- (void)updateLevel3:(NSArray *)level3
{
    if(!self.level3)
        self.level3 = [[StockWatchLevel3V3 alloc] initWithFrame:CGRectMake(0, 0, self.container.frame.size.width, self.container.frame.size.height)];
    
    KiStockData *data = [[MarketData sharedInstance] getStockDataByStock:self.stockCode];
    [self.level3 updateLevel3:level3 data:data];
}

- (void)updateTrade:(NSArray *)trade
{
    if(!self.trade)
        self.trade = [[StockWatchTradeV3 alloc] initWithFrame:CGRectMake(0, 0, self.container.frame.size.width, self.container.frame.size.height)];
    
    KiStockData *data = [[MarketData sharedInstance] getStockDataByStock:self.stockCode];
    [self.trade updateTrade:trade data:data];
}
