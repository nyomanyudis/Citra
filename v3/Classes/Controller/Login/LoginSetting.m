...
#import "LoginSetting.h"
#import "UIButton+Addons.h"
#import "Agent.h"
#import "SystemAlert.h"
#import "UITextField+Addons.h"

@interface LoginSetting () <UITableViewDataSource>

@property (weak,nonatomic) IBOutlet UIButton *sideBack;
@property (weak,nonatomic) IBOutlet UITableView *table;
@property (strong,nonnull) SettingApp *settingApp;

@end

@implementation LoginSetting
...
-(void) actionbuttonSave:(id)sender {
    ...
    if([LoginSetting saveSetting:set]){
...
- (void)viewDidLoad {
    ...
    [self.sideBack addTarget: self
                   action: @selector(backToLoginStep1:)
         forControlEvents: UIControlEventTouchUpInside];
    
    [self.sideBack clearBackground];
    
    self.table.dataSource = self;
    self.table.separatorColor = [UIColor clearColor];
    //
    self.settingApp = [LoginSetting loadSetting];

    
}

- (void) backToLoginStep1:(id)sender {
    [self performSegueWithIdentifier:@"IdentifierLoginStep1" sender:nil];
}
