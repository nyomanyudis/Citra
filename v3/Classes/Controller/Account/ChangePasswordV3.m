...
#import "ChangePasswordV3.h"

#import "EntryCashWithdrawCell.h"
#import "SystemAlert.h"
#import "UITextField+Addons.h"

@interface ChangePasswordV3 () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate>
...
@implementation ChangePasswordV3
...
- (IBAction)submitCash:(id)sender
{
    ...
    if(pin.length <= 0)
        [self errorAlert:@"OLD Password is Empty"];
    else if(pin1.length <= 0)
        [self errorAlert:@"NEW Password is Empty"];
    else if(pin1.length < 6)
        [self errorAlert:@"NEW Password minimun length is 6"];
    else if(pin2.length <= 0)
        [self errorAlert:@"REPEAT New Password is Empty"];
    else if(![pin2 isEqualToString:pin1])
        [self errorAlert:@"New Password Doesn't Match"];
    else
        [[MarketTrade sharedInstance] changePassword:pin newPasswd:pin1];
...
{
    if(ok && tm) {
        if(tm.recType == RecordTypeChangePassword && tm.recStatusReturn == StatusReturnResult) {
            SIAlertView *alert = [SystemAlert alert:@"Congratulation" message:@"Password Changed" handler:nil button:@"OK"];
            [alert show];
        }
    }
...
//            cell.inputField.keyboardType = UIKeyboardTypeNumberPad;
            ...
            cell.label.text = @"REPEAT New Password";
//            cell.inputField.keyboardType = UIKeyboardTypeNumberPad;
            cell.inputField.textAlignment = NSTextAlignmentRight;
            cell.inputField.secureTextEntry = YES;
            //[cell.inputField becomeFirstResponder];
