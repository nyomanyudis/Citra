...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        [colors addObject:[UIColor colorWithHexString:@"0x96874e"]];
    }
...
//    if (!self.markedDates || isSelectedDatePreviousMonth || isSelectedDateNextMonth) return;
    ...
    //Draw markings
    if (nil != self.markedDates || !isSelectedDatePreviousMonth || !isSelectedDateNextMonth) {
        for (int i = 0; i<[self.markedDates count]; i++) {
            ...
            int targetX = targetColumn * (kVRGCalendarViewDayWidth+2) + 5;
            int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2) + 5;
            
            CGRect rectangle = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth-8,kVRGCalendarViewDayHeight-8);
            CGContextAddRect(context, rectangle);
            UIColor *color;
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            if (todayMarked) {
                CGRect rectangle = CGRectMake(targetX+5,targetY+5,kVRGCalendarViewDayWidth-8,kVRGCalendarViewDayHeight-8);
                CGContextAddRect(context, rectangle);
                UIColor *color = (UIColor *)[markedColors objectAtIndex:0];
...
//        CGRect rectangle = CGRectMake(targetX,targetY,kVRGCalendarViewDayWidth+2,kVRGCalendarViewDayHeight+2);
////        CGRect rectangle = CGRectMake(targetX, targetY, kVRGCalendarViewDayWidth-2, kVRGCalendarViewDayHeight-2);
//        CGContextAddRect(context, rectangle);
//        
//        UIColor *color;
